...
void encode_one_macroblock_high (Macroblock *currMB)
{
 ...
 int SkipIntra_Lindan=0;
  ...
  if (!intra)
  {
	...
	//if(JudgeofSkipInterByLindan[currMB->mb_x+22*currMB->mb_y]==1)
	  //  enc_mb.valid[P8x8]=0;
//Modified by Lindan for Inter skip
  ...
  for (currMB->c_ipred_mode = chroma_pred_mode_range[0]; currMB->c_ipred_mode<=chroma_pred_mode_range[1]; currMB->c_ipred_mode++)
  {
    ...
    {//Modified by Lindan for Intra skip
       /*if(currMB->mb_y>0&&currMB->mb_x>0&&currMB->mb_y<21&&index==5)
    { a[0]=RDcost_Lindan[22*currMB->mb_y+currMB->mb_x-1];
	  a[1]=RDcost_Lindan[22*(currMB->mb_y-1)+currMB->mb_x-1];
	   a[2]=RDcost_Lindan[22*(currMB->mb_y-1)+currMB->mb_x];
	    a[3]=RDcost_Lindan[22*(currMB->mb_y-1)+currMB->mb_x+1];
		min_Lindan=a[0];
		for(i_Lindan=1;i_Lindan<4;i_Lindan++)
		{if (a[i_Lindan]<min_Lindan)
		  min_Lindan=a[i_Lindan];
		}
     if(min_Lindan>=SSE_Lindan)
	 break;
	 //SkipIntra_Lindan=1;
	}
	//Modified by Lindan*/
      ...
      if (enc_mb.valid[mode])
      {
       ...
       /* RDcost_Lindan[22*currMB->mb_y+currMB->mb_x]=currMB->min_rdcost;
	    if(index<=4)
	    {SSE_Lindan=currMB->min_dcost;}
		Modified by Lindan for Intra skip*/
      }
